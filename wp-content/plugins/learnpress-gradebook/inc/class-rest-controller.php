...
class LP_Gradebook_Rest_Controller {
		...
		try {
			...
			if ( ! empty( $results ) ) {
				foreach ( $results as $result ) {
					...
					$course_data    = learn_press_get_user( $user_id )->get_course_data( absint( $course_id ) );
		...
		try {
			...
			if ( $result ) {
				foreach ( $result as $value ) {
					...
					$course_data    = learn_press_get_user( $user_id )->get_course_data( absint( $course_id ) );
					$course_results = $course_data->calculate_course_results();
					$get_result     = LP_User_Items_Result_DB::instance()->get_result( $course_data->get_user_item_id() );

					$value['average'] = ! empty( $get_result['result'] ) ? round( (float) $get_result['result'], 2 ) . '%' : '0%';
	...
	public function get_items( $request ) {
		...
		try {
			...
			$course_items = LP_Gradebook_Database::instance()->get_item_id_in_user_items( $course_id, $student_id, $status, $item_type );
	...
	public function lp_gradebook_get_questions( $request ) {
		...
		try {
			...
			$user_item_id = learn_press_get_user_item_id( $student_id, $quiz_id, $course_id );

			if ( ! $user_item_id ) {
				$user         = learn_press_get_user( $student_id );
				$course_data  = $user->get_course_data( $course_id );
				$user_item_id = $course_data->get_item( $quiz_id )->get_user_item_id();
			}
			...
			$result_new = LP_Gradebook_Database::instance()->lp_gradebook_get_qizz_result( $quiz_id, $student_id );
			...
			$response->data->result      = $result_new;
	...
	public function get_chart_questions( $request ) {
		$quiz_id    = $request->get_param( 'quizzID' );
		$student_id = $request->get_param( 'studentID' );
		...
		try {
			...
			$count_answer_true  = (int) $result->question_correct;
			$count_answer_false = (int) $result->question_count - $count_answer_true;
