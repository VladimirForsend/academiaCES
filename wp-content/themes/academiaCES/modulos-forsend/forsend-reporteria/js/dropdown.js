   ...
   var drop = function(info){var o = {
       ...
       this.selected.forEach(function(select, index){ 
           ...
           var childCloseHTML = $.template(
               '<i class="material-icons btnclose" onclick="remove_selector('+select.index+');" data-index="'+select.index+'">&#xe5c9;</i></span>')
       ...
       this.options.forEach(function(option, index){
           var childHTML = $.template(
               '<a data-index="'+index+'" onclick="add_selector('+index+');" class="'+option.state+'">'+ option.html +'</a>')
...
function add_selector(data){
    if(data == "0")
        index = 100;
    else
        index = Math.floor(data) -1 ;
    jQuery('#myMulti [value="'+index+'"]').attr("selected","selected");
}
function remove_selector(data){
    ...
    if(data == "0")
        ...
        index = Math.floor(data) -1 ;
    jQuery('#myMulti [value="'+index+'"]').removeAttr("selected");
}